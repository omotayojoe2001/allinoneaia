import { useState } from "react";
import { ChevronRight } from "lucide-react";
import EmailAnalytics from "./EmailAnalytics";
import EmailSequences from "./EmailSequences";
import BulkEmailSender from "./BulkEmailSender";
import CRMEmailTriggers from "./CRMEmailTriggers";
import EmailDeliverability from "./EmailDeliverability";
import UnsubscribeManagement from "./UnsubscribeManagement";

const EmailMarketingHub = () => {
  const [active, setActive] = useState("analytics");

  const tabs = [
    { id: "analytics", label: "Analytics", component: EmailAnalytics },
    { id: "sequences", label: "Sequences", component: EmailSequences },
    { id: "bulk", label: "Bulk", component: BulkEmailSender },
    { id: "triggers", label: "Triggers", component: CRMEmailTriggers },
    { id: "deliverability", label: "Deliverability", component: EmailDeliverability },
    { id: "unsubscribe", label: "Unsubscribe", component: UnsubscribeManagement },
  ];

  const ActiveComponent = tabs.find(t => t.id === active)?.component;

  return (
    <div className="flex-1 flex flex-col">
      <div className="bg-background px-6 py-3 flex gap-1 border-b overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActive(tab.id)}
            className={`px-4 py-2 text-sm font-medium rounded-t transition-all flex items-center gap-1 whitespace-nowrap ${
              active === tab.id
                ? "bg-primary text-primary-foreground shadow-sm"
                : "text-muted-foreground hover:text-foreground hover:bg-accent"
            }`}
          >
            {tab.label}
            {active === tab.id && <ChevronRight className="w-3 h-3" />}
          </button>
        ))}
      </div>
      <div className="flex-1 overflow-y-auto">
        {ActiveComponent && <ActiveComponent />}
      </div>
    </div>
  );
};

export default EmailMarketingHub;
